...
describe('GET /todos/:id',()=>{
  ...
  it('should not get todo from other user',(done)=>{
    request(app)
...
describe('POST user/login',()=>{
    ...
    it('should reject invalid login',(done)=>{
      ...
      .end((err,res)=>{
        ...
        User.findById(users[1]._id).then((user)=>{
          expect(user.tokens[0].token).not.toEqual(users[0].tokens[0].token);
          done();
        }).catch((e)=>done(e));
      });
    });
...
describe('Delete users/me/token',()=>{
  it('should delete token elemnt from user',(done)=>{
    ...
    .end((err,res)=>{
        ...
        User.findById(users[1]._id).then((user)=>{
          expect(user.tokens.length).not.toEqual(users[0].tokens[0].token);
          done();
        }).catch((e)=>done(e));
    });
  });
